...
for ($start=strtotime(date("Y-m-d H:00:00",$dateNextLogEntry)); $start<$currentTime; $start+=3600) {
	...
	$sql = sprintf("SELECT useragent, ip, SUBSTRING_INDEX(SUBSTRING_INDEX(resource,'?',1),'index.',1) AS resource FROM %s WHERE date>='%s' AND date<'%s' AND ID<='%s' GROUP BY resource",
		$engineDB->escape("log"),
		$engineDB->escape($start),
		$engineDB->escape($end),
		$engineDB->escape($latestLogEntry)
	...
	foreach ($data as $resource => $V) {
		...
		$sql = sprintf("SELECT ID FROM %s WHERE year='%s' AND month='%s' AND day='%s' AND hour='%s' AND resource='%s'",
		...
		if ($sqlResult['affectedRows'] > 0) {
			...
			$sql = sprintf("UPDATE %s SET mobilevisits='%s', nonmobilevisits='%s', mobilehits='%s', nonmobilehits='%s' WHERE ID='%s'",
				$engineDB->escape("logHits"),
			...
			$sql = sprintf("INSERT INTO %s (year, month, day, hour, mobilevisits, nonmobilevisits, mobilehits, nonmobilehits, resource) VALUES ('%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s')",
				$engineDB->escape("logHits"),
				date("Y",$start),
				date("m",$start),
				date("d",$start),
...
for ($start=strtotime(date("Y-m-01 00:00:00",$dateNextLogEntry)); $start<$currentTime; $start=strtotime('+1 month',$start)) {
	...
	$sql = sprintf("DELETE FROM %s WHERE year='%s' AND month='%s'",
	...
	$sql = sprintf("SELECT useragent, referrer, COUNT(ID) AS hits, SUBSTRING_INDEX(SUBSTRING_INDEX(resource,'?',1),'index.',1) AS page FROM %s WHERE date>='%s' AND date<'%s' AND ID<='%s' GROUP BY page, referrer, useragent ORDER BY hits DESC",
		$engineDB->escape("log"),
		$engineDB->escape($start),
		$engineDB->escape($end),
		$engineDB->escape($latestLogEntry)
	...
	if ($sqlResult['result']) {
		while ($row = mysql_fetch_array($sqlResult['result'], MYSQL_ASSOC)) {
			...
			$sql = sprintf("INSERT INTO %s (year, month, mobilehits, nonmobilehits, url, referrer) VALUES ('%s', '%s', '%s', '%s', '%s', '%s')",
				$engineDB->escape("logURLs"),
				date("Y",$start),
				date("m",$start),
...
for ($start=strtotime(date("Y-m-01 00:00:00",$dateNextLogEntry)); $start<$currentTime; $start=strtotime('+1 month',$start)) {
	...
	// Create list from log table
	$sql = sprintf("SELECT useragent, ip, SUBSTRING_INDEX(SUBSTRING_INDEX(resource,'?',1),'index.',1) AS resource FROM %s WHERE date>='%s' AND date<'%s' AND ID<='%s'",
		$engineDB->escape("log"),
		$engineDB->escape($start),
		$engineDB->escape($end),
		$engineDB->escape($latestLogEntry)
	...
	if ($sqlResult['result']) {
		...
		foreach ($list as $resource => $userAgents) {
			foreach ($userAgents as $array) {
				...
				$sql = sprintf("INSERT INTO %s (year, month, resource, browser, nonHuman, onCampusCount, offCampusCount, os, mobile) VALUES ('%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s')",
					$engineDB->escape("logBrowsers"),
					date("Y",$start),
					date("m",$start),
					$engineDB->escape($resource),
