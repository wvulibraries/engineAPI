...
$sql = sprintf("SELECT SUM(mobilehits) AS totalMobileHits, SUM(nonmobilehits) AS totalNonmobileHits, COUNT(DISTINCT url) AS totalURLs FROM %s WHERE %s UNIX_TIMESTAMP(CONCAT(year,'-',month,'-01 00:00:00'))>='%s' AND UNIX_TIMESTAMP(CONCAT(year,'-',month,'-01 00:00:00'))<'%s' AND referrer='NULL'",
...
<table class="sortable">
	...
	<tbody>
		<?
		$sql = sprintf("SELECT SUM(mobilehits) AS mobilehits, SUM(nonmobilehits) AS nonmobilehits, url FROM %s WHERE %s UNIX_TIMESTAMP(CONCAT(year,'-',month,'-01 00:00:00'))>='%s' AND UNIX_TIMESTAMP(CONCAT(year,'-',month,'-01 00:00:00'))<'%s' AND referrer='NULL' GROUP BY url ORDER BY SUM(mobilehits+nonmobilehits) DESC",
			$engineDB->escape("logURLs"),
			$sqlSite,
			$engineDB->escape($monthStart),
			$engineDB->escape($monthEnd)
