...
class fileList {
	...
	public function applyTemplate($template=NULL) {
		...
		foreach ($this->files as $key => $value) {
			...
			$output .= displayTemplate($tempContent);
		}
...
function filelist($dir,$template) {
	
	global $engineVars;
	
	$engineVars['fileList'] = new fileList($dir);
	
	//$output   = $engineVars['fileList']->printObject();
	//$output .= "dir == $template";
	
	$output = $engineVars['fileList']->applyTemplate($engineVars['currentTemplate']."/".$template);
