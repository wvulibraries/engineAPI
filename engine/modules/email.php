...
class mailSender {
	...
	private function performBulkSend($sendID,$dbInfo) {
		...
		return exec("php /home/ereserves/public_html/engineIncludes/emailSendBulk.php -id=$sendID -d=".$dbInfo['database']." -t=".$dbInfo['table']." -u=".$dbInfo['username']." -p=".$dbInfo['password']." -s=".$dbInfo['server']." -P=".$dbInfo['port']." -f=".$dbInfo['fileTable']." >/dev/null &");
		
	}
	
/*	public function sendEmailBulk($sender,$recipient,$subject,$body) {
		
		if (is_array($recipient)) {
			$recipient = escapeshellarg(implode(",",$recipient));
		}
		else {
		...
		$result = exec("php /home/ereserves/public_html/engineIncludes/emailSendBulk.php $recipient $sender $subject $body >/dev/null &#038;");
