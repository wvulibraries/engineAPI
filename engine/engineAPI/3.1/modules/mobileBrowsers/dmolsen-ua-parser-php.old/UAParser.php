...
require(dirname(__FILE__)."/lib/spyc-0.5/spyc.php");
...
class UA {
	...
	*/
	public function parse() {
		...
		self::$regexes = Spyc::YAMLLoad(dirname(__FILE__)."/resources/user_agents_regex.yaml");
	...
	*/
	private function log($data) {
		...
		$fp = fopen(dirname(__FILE__)."/log/user_agents.log", "a");
		fwrite($fp, $jsonData."\r\n");
		fclose($fp);
	}
	...
	*/
	public function get() {
		if ($data = @file_get_contents("http://ua-parser.googlecode.com/svn/trunk/resources/user_agent_parser.yaml")) {
			copy(dirname(__FILE__)."/resources/user_agents_regex.yaml", dirname(__FILE__)."/resources/user_agents_regex.".date("Ymdhis").".yaml");
			$fp = fopen(dirname(__FILE__)."/resources/user_agents_regex.yaml", "w");
			fwrite($fp, $data);
			fclose($fp);
		}
	}
