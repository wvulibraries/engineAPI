...
class engineDB {
	...
	function __construct($user,$passwd,$server,$port,$db,$die=TRUE) {
		...
		if (isset($user) && isset($passwd) && isset($db)) {
			...
			if (!isset(self::$transactionCount[$this->md5ConnName])) {
				self::$transactionCount[$this->md5ConnName] = 0;
			}
		}
	}
	...
	public function transBegin($table=NULL, $autocommit=FALSE) {
		...
		if(self::$transactionCount[$this->md5ConnName] > 0){
			self::$transactionCount[$this->md5ConnName]++;
			return TRUE;
		}
	...
	public function transEnd($autocommit=FALSE) {
		...
		// Neither Rollback or Commit has been called. Just end.
		else {
			// whatever
		}
	 ...
	 * @param bool $inclPrimary -- if FALSE does not return the primary key
	 * @param bool $returnArray -- if set to true returns an array, otherwise comma delimited string
	 * @return array|null
	 */
	 public function listFields($tblName,$inclPrimary=TRUE,$returnArray=TRUE,$skipList=NULL){
		/*
		* Using tableInfo() allows us to examine temp tables since it relies on DESCRIBE `table` as
		* opposed to the information_schema (which only includes defined tables)
		*/
		...
		if($tblInfo){
			...
			foreach($tblInfo['fields'] as $field){
				// Skip primary key
				if($field['key'] == 'PRI' and !$inclPrimary){
					continue;
				}

				if (!isnull($skipList) && is_array($skipList) && in_array($field['field'],$skipList)) {
					continue;
				}
