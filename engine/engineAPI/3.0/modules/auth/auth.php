...
//		if($permissionOrigin == auth::GLOBAL_PERMISSION) $inheritable = TRUE;
        ...
        if($this->autoPropagate){
            errorHandle::newError(__METHOD__."() - Auto Propagate", errorHandle::DEBUG);
            if($this->propagateInheritance()){
                errorHandle::newError(__METHOD__."() - Commit", errorHandle::DEBUG);
				$this->db->transCommit();
				$this->db->transEnd();
				return TRUE;
			}else{
	...
	public function revoke($entity,$permissionName,$permissionOrigin=auth::GLOBAL_PERMISSION,$policy='allow')
	{
        ...
        if($permissionName == '*'){
            // Remove ALL the entities' authorizations
            $dbAllAuths = $this->db->query(sprintf("SELECT * FROM `%s` WHERE `authEntity`='%s' AND `authObjectID`='%s'",
                $this->db->escape($this->tblAuthorizations),
                $this->db->escape("$entity"),
                $this->db->escape($this->objectID)));
		...
		if(!isset($this->authSearchTrees["$object"])){
            ...
            if(!$dbAuthorizations['error']){
                while($row = mysql_fetch_assoc($dbAuthorizations['result'])){
                    ...
                    $this->authSearchTrees["$authObjectID"][ $authEntity ][] = $row;
                }
            }else{
			...
			foreach($searchTree as $entityID => $authorizations){
				foreach($authorizations as $authorization){
					if(is_null($permissionObject)){
						if($authorization['permissionName'] == auth::formatName($permissionName) and ($authorization['permissionObject'] == auth::formatName(auth::GLOBAL_PERMISSION) or $authorization['permissionObject'] == auth::formatName("$object"))){
