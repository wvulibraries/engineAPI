...
class revisionControl {
	...
	// $ID : value of $primary
	public function insertRevision($ID) {
	...
	public function revertSubmit($ID,$ID2) {
		...
		$prod2RevResult = $this->insertRevision($ID);

		if ($prod2RevResult === FALSE) {
			errorHandle::newError("Error Copying row from production to revision tables, SQL ERROR: ".$sqlResult['error'], errorHandle::DEBUG);
		...
		if ($rev2ProdResult === FALSE) {
			...
			errorHandle::newError("Error Copying row from revision to production tables, SQL ERROR: ".$sqlResult['error'], errorHandle::DEBUG);
			errorHandle::errorMsg("Error reverting to previous revision.");
