	...
	*/
	...
	public function insert() {
		...
		foreach ($this->fields as $I) {
			...
			if ($I['type'] != "multiselect" && $I['type'] != "checkbox" && $I['disabled'] === FALSE) {
	...
	private function buildInsertUpdateString($and=FALSE) {
		...
		foreach ($this->fields as $I) {
			...
			$temp[] = sprintf("%s='%s'",
				...
				($I['type'] == "password")?mhash($this->passwordHash,$engine->cleanPost['MYSQL'][$I["field"]."_insert"]):$engine->cleanPost['MYSQL'][$I["field"]."_insert"]);
		}
		foreach ($this->hiddenFields as $I) {
			if($I['disabled'] === TRUE) {
				continue;
	...
	private function buildFieldValueInsert() {
		...
		foreach ($this->fields as $I) {
			...
			$value = ($I['type'] == "password")?bin2hex(mhash($this->passwordHash,$engine->cleanPost['MYSQL'][$I["field"]."_insert"])):$engine->cleanPost['MYSQL'][$I["field"]."_insert"];
