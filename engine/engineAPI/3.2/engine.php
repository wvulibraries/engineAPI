...
class EngineAPI{
	...
	public function eTemplate($func, $value = NULL) {
		...
		if ($func == "load") {
			...
			$templateDir = $engineVars['tempDir'] . "/" . $value;
		...
		if ($func == "include") {
			...
			switch ($value) {
				case "header":
					$filePath = $this->template . "/templateHeader.php";
					break;
	
				case "footer":
					$filePath = $this->template . "/templateFooter.php";
					break;
	...
	private function engineLog($type = "access", $function = NULL, $message = NULL) {
		...
		// Escape values
		$date      = $engineDB->escape($date);
		$ip        = $engineDB->escape($ip);
		$referrer  = $engineDB->escape($referrer);
		$resource  = $engineDB->escape($resource);
		$queryStr  = $engineDB->escape($queryStr);
		$useragent = $engineDB->escape($useragent);
		$function  = $engineDB->escape($function);
		$type      = $engineDB->escape($type);
		$message   = $engineDB->escape($message);
		...
		$result = $engineDB->query($query);
	
		if ($result === FALSE) {
			error_log("Failed to log to database: " . $engineDB->escape(mysqli_error($engineDB->dbLink)));
			return FALSE;
		}
