...
class errorHandle
{
    ...
    private static function getUserAgent()
    {
        if (isCLI()) {
            ...
            $sudoUser = isset($_SERVER['SUDO_USER']) ? $_SERVER['SUDO_USER'] : null;
    
            if ($sudoUser !== null) {
                return sprintf('%s (via sudo by %s) from %s', $user, $sudoUser, $ip);
            } else {
    ...
    public static function errorMsg($msg)
    {
        ...
        $formattedMsg = sprintf('<%s class="%s">%s</%s>', self::$uiSpanElement, self::$uiClassError, $msg, self::$uiSpanElement);
        return $formattedMsg;
    }
    ...
    public static function successMsg($msg)
    {
        ...
        $formattedMsg = sprintf('<%s class="%s">%s</%s>', self::$uiSpanElement, self::$uiClassSuccess, $msg, self::$uiSpanElement);
        return $formattedMsg;
    }
    ...
    public static function warningMsg($msg)
    {
        ...
        $spanElement = htmlspecialchars(self::$uiSpanElement);
        $classAttribute = htmlspecialchars(self::$uiClassWarning);
        $escapedMsg = htmlspecialchars($msg);
        
        return "<$spanElement class=\"$classAttribute\">$escapedMsg</$spanElement>";
    }

    public static function prettyPrint($type = "all")
    {
        // Check if EngineAPI class exists
        if (!class_exists('EngineAPI', false)) {
            return ''; // No EngineAPI, return empty string
        }
    
        $engine = EngineAPI::singleton();
        $output = '<ul class="errorPrettyPrint">';
        
        // Check if errorStack for the specified type exists and is an array
        if (!isset($engine->errorStack[$type]) || !is_array($engine->errorStack[$type])) {
            return ''; // Return empty string if errorStack is not valid
        }
    
        // Determine the CSS class based on the error type
        switch ($type) {
            case "all":
                $errorTypeClass = [
                    self::ERROR => self::$uiClassError,
                    self::SUCCESS => self::$uiClassSuccess,
                    self::WARNING => self::$uiClassWarning,
                ];
                $errorStack = isset($engine->errorStack['all']) ? $engine->errorStack['all'] : array();
                break;
            case self::ERROR:
                $errorTypeClass = "errorMessage";
                $errorStack = $engine->errorStack[$type];
                break;
            case self::SUCCESS:
                $errorTypeClass = "successMessage";
                $errorStack = $engine->errorStack[$type];
                break;
            case self::WARNING:
                $errorTypeClass = "warningMessage";
                $errorStack = $engine->errorStack[$type];
                break;
            default:
        ...
        // Generate the HTML output based on error type
        foreach ($errorStack as $error) {
            $class = is_array($errorTypeClass) ? $errorTypeClass[$error['type']] : $errorTypeClass;
            $output .= "<li><span class=\"$class\">{$error['message']}</span></li>";
        }
    ...
    {
        if (!class_exists('EngineAPI', false)) {
            return; // No EngineAPI available, so exit early
        }
