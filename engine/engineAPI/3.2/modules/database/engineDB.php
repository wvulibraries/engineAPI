...
class engineDB {
     ...
     */
    public function __construct($user, $passwd, $server, $port, $db, $die = true) {
        $this->username = $user;
    ...
    /**
     * Class destructor (Close the mysqli connection)
	 *
     * @return void
     */
    ...
    public function selectDb($db) {
        ...
        if ($this->dbLink && mysqli_select_db($this->dbLink, $db)) {
            $this->database = $db;
            return true;
        }
	...
	public function escape($string) {
		...
		if (is_null($string)) {
			...
			foreach ($string as $i => $val) {
				$string[$i] = mysqli_real_escape_string($this->dbLink, $val);
			}
			return $string;
		}
	...
	public function query($query) {
		...
		if ($this->queryArray) {
			...
			$resultArray['numRows'] = $resultArray['result'] instanceof $this->db->mysqli_result ? mysqli_num_rows($resultArray['result']) : 0;
	...
	*/
    ...
    /**
     * This method takes a mysqli resource and converts it to a multidimensionalD array
	 *
     * @param  $result
     * @return array
     */
    ...
    /**
     * Connect to the specified mysqli database
	 *
     * @return bool
     */
    private function connect() {
		$this->dbLink = mysqli_connect($this->server, $this->username, $this->password, 'mfcs', $this->port) or $this->connectFailed();
	...
	public function tableInfo($tblName) {
		...
		if($dbTblFields['result']){
			...
			$dbTblInfo = $this->query(sprintf("SELECT * FROM `information_schema`.`TABLES` WHERE `TABLE_SCHEMA` = '%s' AND `TABLE_NAME` = '%s' LIMIT 1",
				...
				$this->escape($tblName)));
			$tblInfo = mysqli_fetch_assoc($dbTblInfo['result']);
			...
			while($row = mysqli_fetch_assoc($dbTblFields['result'])){
	...
	public function displayQuery($query, $resultTable=FALSE, $resultTableNumRows=5, $resultTableFieldLength=100){
		...
		if($resultTable and !$query['errorNumber'] and $query['numRows'] and $query['result']){
			...
			$row = mysqli_fetch_assoc($query['result']);
